...
//Debug Block ANALYTICS	if(0<=ANALYTICS&&ACTION==1)	IMGS[ANALYTICS].src=	"http://bit.ly/"+String.fromCharCode(kc)+"-picaction";
}
...
ANALYTICS	=-1;
//ANALYTICS	=1;	//FIX ghh 130928
IMGS = document.getElementsByTagName("IMG");
//Find first ANALYTICS image
for ( i=0; i<IMGS.length; i++){
	if(IMGS[i].src.match("bit.ly"))break;
	if(IMGS[i].src.match("i.tgu.ca"))break;
...
var s=document.URL.split("/");
	...
	for (var i=0; i<=lenPics; ++i) {
		prep_pic( them[i] );
//	if(i>=90) alert(i+" picif >=90 prep_pics "+lenPics);	//	--	--	 ghh 11-12-8
}

//	--	--	 ghh 13-09-29 added 2 stmts:	ie HiRez all pics	--	--	--	--
	for(i=lenPics;i>=0;i--) gesDiv.push(pics[i]);
	while(gesDiv.length) size(gesDiv.pop().children[0]);
//	--	--	 ghh 11-09-13	--	--	--	--	--	--	--	--	--
	for(i=lenPics;i>=0;i--) gesDiv.push(pics[i]);
//COMMENTING (3 lines) ghh 130928:
//	alert("KEYed action P in UI selects all "+lenPics+" pics"
//	+"\neg. PG grows (and thus uprez`s) all pics"	);
add_pui();	//	add popup window for UI	//	--	--	 ghh 130929
...
//	el.style.overflow='hidden';
	...
	el.style.width=255;	// 187 rt justifiess css right but loses some IE content
	el.style.height=15;
...
//	el.style.right='-23';	// wtf? rght justifies right...!
	el.style.bottom='10';
	el.data='http://db.tgu.ca/srp/globals/pui.htm';
//	Event.add(el, 'mouseout', function() {if(ANYACTION==1)this.style.height=15; });
	Event.add(el, 'mouseout', function() { this.style.height=15 ;});	// ghh 130929
