...
var foo=c.tweet.replace(/\%2/g,"\$2")
foo=((0<=foo.indexOf('%')) ? foo.replace(/\%/g,"<br/>"):((1<foo.indexOf('\\'))? foo.replace(/\\/g,"<br/>") : ( (0>foo.indexOf('http')) ? foo.replace(/\//g,"<br/>") : foo ) ))
foo=foo.replace(/\$2/g,"\%2")

//	foo=foo.replace(/\#/g," #");

/*
testing @TweitKu testing#poem #poem#poem #poem end

a target="_blank" class="twtr-hashtag" href="http://twitter.com/search?q=%23poem">#
*/

foo=foo+"<br/>"+foo.substring(1+foo.indexOf('<a'));
c.tweet=foo;	/*.replace(/\#/g," \%23")	*/
...
TweitKu	%+c.user
8001344337	%+c.id
http://twitter.com/search?q=%23poem
